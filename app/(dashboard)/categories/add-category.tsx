"use client";
import { addCategory } from "@/app/_apicalls/category";
...
const AddCategory = () => {
  ...
  const { getCategories, setSubCategories, setChildCategoires } =
    useCategoryStore();
  const closeModal = () => {
    if (searchParam.get("type") === "category") {
  ...
  return (
    <>
      <Transition appear show={searchParam.has("createCategory")} as={Fragment}>
                ...
                <Dialog.Panel className=" rounded-xl    border-white dark:border-slate-700 bg-white dark:bg-black ">
                  ...
                  <div className="flex justify-between md:flex-row  shadow-lg p-3  mx-auto w-full md:min-w-[700px] items-center ">
                    <p className="flex items-center">
                      {" "}
                      <MdOutlineCategory className="w-5 h-5 mr-2" /> Add{" "}
